...
[芯片设计进阶之路——门控时钟 - 知乎](https://zhuanlan.zhihu.com/p/139363948)

[门控时钟设计001：什么是门控时钟？如何产生门控时钟？-CSDN博客](https://blog.csdn.net/icxiaoge/article/details/80792819)

A：芯片功耗组成中，有高达40%甚至更多是由时钟树消耗掉的。这个结果的原因也很直观，因为这些时钟树在系统中具有最高的切换频率，而且有很多时钟buffer，而且为了最小化时钟延时，它们通常具有很高的驱动强度。此外，即使输入和输出保持不变，接收时钟的触发器也会消耗一定的功耗。而且这些功耗主要是动态功耗。
...
Q11：1个16K*16位的存储器，其地址线和数据线的总和是

A11：2^n=16K。总和为N+16（位）。所以总和为14+16=30。

Q33：UVM中transaction类常用的方法有

A. do_unpacked() B.compare() C.do_pack() D.run_phase()

A33：ABC

Q22：以下不符合低功耗设计要求的是()
A.在满足性能要求的前提下，尽量采用高频率的时钟信号
B.FPGA全功能版本在产品适用温度范围内，结温不超过阀值的90%
C.器件的所有输出管脚都需要进行电流强度的约束
D.在满足要求情况下驱动电流做到最小

A22：A

Q39：有关代码覆盖率的描述哪个是错误的()
A.assign C=A&&B; 该语句可被语句覆盖分析
B.语句覆盖统计范围是过程赋值语句，包括阻塞赋值和非阻塞赋值
C.分支覆盖可以对子表达式的组合进行分析
D.条件覆盖可以对复杂控制结构中的布尔表达式为真和为假的情况进行分析
...
Q36：verilog中，以下关于function的说法正确的是()
A.function调用既能出现在过程块中，也能出现在assign连续赋值语句中
B.function的调用不能单独作为一条语句出现，它只能作为一个操作数出现在调用语句中
C.function不能调用任何其他task
D.function可以有输出端口或双向端口

A36：ABC

Q23：下面哪个后缀名的文件不是FPGA的编程文件() 
A. rbf B. sof C. bit D. ucf

A23：
A. rbf：
是Altera（现Intel）FPGA的编程文件格式（Raw Binary File），用于串行配置FPGA。
✅ 属于FPGA编程文件。
B. sof：
是Altera（现Intel）FPGA的编程文件格式（SRAM Object File），用于通过JTAG或配置芯片加载FPGA。
✅ 属于FPGA编程文件。
C. bit：
是Xilinx FPGA的编程文件格式（Bitstream File），用于配置Xilinx FPGA器件。
✅ 属于FPGA编程文件。
D. ucf：
是Xilinx FPGA的约束文件（User Constraints File），用于指定引脚分配、时序约束等设计约束，不是编程文件。
ucf文件是设计输入文件，需通过综合工具处理生成bit文件后才能用于编程FPGA。
❌ 不属于FPGA编程文件。

Q18：关于Serdes，描述错误的是()
A.FPGA器件都包含至少-对serdes管脚
B.发送和接收方向可以单独使用
C.serdes是一种高速串行接口
D.必须分配指定的管脚位置
...
Q26：表达式Y=(A?B:C)，如果A为x态或者z态，B=1100，C=1000，则Y的值为多少
A.1x11
B.1000
C.1100
D.1x00

A26：D。在Verilog中，条件运算符 `? :` 的行为规则如下：
- 如果条件 `A` 为 `1`，则结果等于 `B`。
- 如果条件 `A` 为 `0`，则结果等于 `C`。
- 如果条件 `A` 为 `x`（未知）或 `z`（高阻），则结果的每一位根据 `B` 和 `C` 的对应位决定：
  - 如果 `B` 和 `C` 的对应位相同，则结果该位取该值。
  - 如果 `B` 和 `C` 的对应位不同，则结果该位为 `x`（未知）。

Q30：下列说法中正确的是()
A.跨时钟域的信号要用寄存器打一拍再输出。
B.将单比特异步信号通过两个或多个触发器级联一定可以解决跨时钟域同步问题
C.在某些情况下可以使用超过16个状态的状态机。
D.在电平敏感的存储器件(LATCH)中要使用非阻塞赋值。

A30：C

Q20：Verilog语言能描述的层次从高到低为
A.系统级，算法级，RTL级，开关级，门级
B.系统级，RTL级，算法级，门级，开关级
C.系统级，算法级，RTL级，门级，开关级
D.系统级，RTL级，算法级，开关级，门级

A20：C

Q17：FPGA开发工具综合步骤不支持的输入文件类型是()
A. vhd
B.edf
C. sv
D.vm
...
Q14：在高速数字电路的设计中，下面的描述正确的是
A.串扰的产生与信号上升(或下降)沿电流变化无关
B.去耦电容放的越少越好
C.传输线的特征阻抗与长度有关
D.设计过程中，我们要尽量保证电源和地平面的完整性，尽量减少分割

A14：D。在高速数字电路设计中，各选项的分析如下：

- **A. 串扰的产生与信号上升(或下降)沿电流变化无关**  
  **错误**。串扰（crosstalk）是由信号线间的电磁耦合引起的，其强度与信号的上升/下降沿变化率（即di/dt）直接相关。上升/下降沿越陡峭（变化越快），电流变化率越大，产生的电磁场越强，串扰越严重。因此，串扰的产生与信号上升/下降沿的电流变化密切相关。

- **B. 去耦电容放的越少越好**  
  **错误**。去耦电容用于稳定电源电压、抑制电源噪声，尤其在高速电路中，IC的瞬态电流需求会导致电源轨波动。去耦电容需合理放置（靠近IC电源引脚）且数量充足，以提供低阻抗回流路径。若去耦电容过少，电源噪声会增大，导致信号完整性问题或电路误操作。因此，去耦电容并非“越少越好”，而是需根据设计需求优化数量和位置。

- **C. 传输线的特征阻抗与长度有关**  
  **错误**。传输线的特征阻抗（characteristic impedance）是其固有属性，仅取决于物理结构（如导体宽度、介电常数、导体间距等），与长度无关。特征阻抗是单位长度的参数，对于均匀传输线，无论长度如何，其值恒定。长度影响信号的传播延迟和衰减，但不影响特征阻抗本身。

- **D. 设计过程中，我们要尽量保证电源和地平面的完整性，尽量减少分割**  
  **正确**。在高速数字电路设计中，完整的电源和地平面提供低阻抗的电流回流路径，减少电磁干扰（EMI）和信号完整性问题（如反射、串扰）。电源/地平面分割会导致回流路径不连续，增加环路电感，引发噪声和串扰。设计时应尽量保持平面完整性，仅在必要时（如隔离模拟/数字部分）进行分割，并需谨慎处理回流路径（如使用跨接电容或缝隙桥接）。

Q6：以下关于信号的描述正确的是()
A.傅里叶变换适用于频域信号
B.两个信号的卷积等于它们的傅里叶变换乘积
C.采样频率不得高于信号最高频率的2倍
D.信号按照周期性可以分为连续时间信号和离散时间信号

A6：D

# 小红书查缺补漏

Q1：状态机编码格雷码和独热码各自的优势是什么

A1：二进制码和格雷码是压缩状态编码。 若使用格雷编码，则相邻状态转换时只有一个状态位发生翻转，这样不仅能消除状态转换时由多条状态信号线的传输延迟所造成的毛刺，又可以降低功耗。
独热编码即 One-Hot 编码，又称一位有效编码，其方法是使用N位状态寄存器来对N个状态进行编码，每个状态都由他独立的寄存器位，并且在任意时候，其中只有一位有效。虽然使用较多的触发器，但由于状态译码简单，可减少组合逻辑且速度较快， 这种编码方式还易于修改，增加状态或改变状态转换条件都可以在不影响[状态机](https://so.csdn.net/so/search?q=状态机&spm=1001.2101.3001.7020)的其它部分的情况下很方便地实现。另外，它的速度独立于状态数量。与之相比，压缩状态编码在状态增加时速度会明显下降。

Q2：你认为一个好的电路设计需要什么因素

A2：1.功能正确，先跑通再优化。2.时序收敛。3.可测性分析把关键信号）通过IO引出，用逻辑分析仪抓波形。

Q3：怎么考虑资源和能耗的。怎么优化。

A3：移位代替乘法，资源优化 = 减少寄存器/逻辑门（状态机/组合逻辑）
能耗优化 = 减少翻转（时钟门控/信号打拍）

Q4：128点FFT，7级流水线，能否优化流水线级数，根据什么进行优化？

A4：**FFT流水线优化 = 资源 vs 时钟频率的权衡**
**减少级数 → 面积减，但时钟频率可能降**
**增加级数 → 面积增，但时钟频率可提升**
**必须用时序分析工具验证，结合实际需求决定**

Q5：时序分析和优化怎么做的

A5：**1. 报告违例（`report_timing_summary`找最差slack）**
**2. 定位路径（`report_path`看组合逻辑级数）**
**3. 优化策略（拆组合逻辑+流水线/调整时钟约束）**
**必须用课设数据佐证”**